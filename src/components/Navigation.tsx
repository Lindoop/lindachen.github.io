        ...
        <div className="flex items-center gap-8">
          ...
          >
            <img
              src={Home}
              alt="Home"
              className="h-40 w-auto cursor-pointer hover:scale-105 transition-transform"
            />
          </button>
          ...
          <ul className="hidden md:flex gap-[clamp(1rem,3vw,4rem)] items-center font-body font-bold ml-[15vh]">
            {navLinks.map((link) => (
              <li key={link.id}>
                <button
                  type="button"
                  onClick={() => scrollToWithOffset(link.id)}
                  className="text-black transition-colors cursor-pointer font-body"
                >
        ...
        <div className="fixed inset-0 z-1000 bg-foreground text-white md:hidden flex flex-col items-center justify-center space-y-6">
          {navLinks.map((link) => (
            <button
              ...
              className="text-primary hover:text-secondary text-2xl font-fancy"
            >
              {link.name}
            </button>
          ))}
